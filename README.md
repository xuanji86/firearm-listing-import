...
- Photos taller than wide (after EXIF rotation) block `attach` for that gun (`resolve` flags `PORTRAIT:<n>`); the store's grid and gallery are landscape. Override with `--allow-portrait`.
